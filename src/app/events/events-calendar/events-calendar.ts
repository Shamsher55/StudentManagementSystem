...
export class EventsCalendar implements OnInit {
  ...
  build() {
    ...
    const monthEvents = this.service.getByMonth(this.year, this.month + 1);
    ...
    this.upcomingEvents = this.service.getUpcoming();
    this.allEvents      = this.service.getAll();
  }
  ...
  delete(id: number) { if (confirm('Delete event?')) { this.service.delete(id); this.build(); } }
